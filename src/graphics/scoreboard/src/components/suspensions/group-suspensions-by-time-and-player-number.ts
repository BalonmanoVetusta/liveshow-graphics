...
export declare type PlayerInfoPayloadWithSuspensionLength =
  PlayerInfoPayload & {
...
export function groupSuspensionsByTimeAndPlayerNumber(
  ...
  const suspensions = actions.filter(
    (a) =>
      (a.action === "SUSPENSION" || a.action === "DISQUALIFICATION") &&
      a.team === team
  );

  const suspensionsWithLength: Array<MatchActionSuspensionTime> =
    suspensions.map((s: MatchAction): MatchActionSuspensionTime => {
      (s as MatchActionSuspensionTime).payload.suspensionLength = 1;
      return <MatchActionSuspensionTime>s;
    });

  const groupedSuspensions: Array<MatchActionSuspensionTime> =
    suspensionsWithLength.reduce(
      (acc, suspension: MatchActionSuspensionTime) => {
        if (acc.length === 0 || !Array.isArray(acc)) return [suspension];
        const { number } =
          suspension.payload as PlayerInfoPayloadWithSuspensionLength;
        const previousSuspensions = acc.filter((s) => {
          const previousMatchTimeEndSuspensionTime =
            s.matchTime + suspensionTime;

          return (
            s.payload.number === number &&
            previousMatchTimeEndSuspensionTime > s.matchTime
          );
        });
        ...
        if (previousSuspensions.length > 1)
          throw new Error(
            "More than one previous suspension found for the same player in the reducer"
          );

        const [previousSuspension] = previousSuspensions;
        const previousSuspensionId = previousSuspension.id;
        const previousSuspensionIndex = acc.findIndex(
          (s) => s.id === previousSuspensionId
        );
        ...
        if (previousSuspensionIndex === -1)
          throw new Error(
            "Previous suspension not found while counting suspensions for the same player in the reducer"
          );
